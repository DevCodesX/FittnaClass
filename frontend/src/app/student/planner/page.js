...
function dayLabel(s) {
    ...
    if (s === ts) return 'اليوم';
    if (s === fmt(y)) return 'أمس';
    if (s === fmt(tm)) return 'غداً';
...
function monthLabel(s) {
    return ['يناير','فبراير','مارس','أبريل','مايو','يونيو','يوليو','أغسطس','سبتمبر','أكتوبر','نوفمبر','ديسمبر'][new Date(s + 'T00:00:00').getMonth()];
}
...
function BlockCard({ task, onToggle, onDelete, actionLoading, dragListeners, dragDisabled, isOverlay }) {
    ...
    const typeConfig = {
        ...
        custom: { icon: 'edit_note', color: 'slate-500', label: 'مخصصة', bg: 'bg-slate-50' },
        timer: { icon: 'timer', color: 'amber', label: 'مؤقت', bg: 'bg-amber-light/40' },
        review: { icon: 'replay', color: 'violet', label: 'مراجعة', bg: 'bg-violet-50' },
    };
    if (task.type === 'event') {
        typeConfig.event = isLiveEvent
            ? { icon: 'sensors', color: 'rose', label: 'بث مباشر', bg: 'bg-rose-light/70' }
            : { icon: 'fact_check', color: 'amber', label: 'اختبار', bg: 'bg-amber-light/70' };
    }
        ...
        `}>
            <div className="flex items-start gap-2.5 p-3">
                ...
                <button
                    ...
                    title={dragDisabled ? 'لا يمكن تحريك بلوكات الأحداث' : undefined}
                >
            ...
            <div className="flex-1 p-2 space-y-1.5 overflow-y-auto" style={{ maxHeight: isMobile ? 'none' : '400px' }}>
                ...
                {tasks.length === 0 && (
                    <div className="text-center py-6">
                        ...
                        <p className="text-[11px] text-slate-300 mt-1">لا توجد مهام</p>
                    </div>
                )}
            </div>
...
export default function PlannerPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            toast.error('حدث خطأ في تحميل الخطة.');
        } finally {
            setLoading(false);
        }
    }, [dates]);
    ...
    const toggleTask = async (taskId, currentStatus) => {
        ...
        try {
            ...
            if (gam && gam.points > 0) {
                toast.success(`+${gam.points} نقطة${gam.leveled_up ? ` 🎉 مستوى ${gam.level}!` : ''}`);
                if (gam.new_achievements?.length > 0) {
                    for (const ach of gam.new_achievements) {
                        setTimeout(() => toast.success(`🏆 إنجاز: ${ach.title}`), 500);
                    }
                }
            }
    ...
    const deleteTask = async (taskId) => {
        const task = findTaskById(taskId);
        if (task?.type === 'event') {
            toast.error('لا يمكن حذف بلوك الحدث من المخطط.');
            return;
        }
        ...
        <div className="py-5 sm:py-8 animate-fade-in" dir="rtl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-0.5">خطط لأسبوعك الدراسي بذكاء</p>
                </div>
                <div className="flex items-center gap-2">
                    <button
                ...
                <div className="mb-4 bg-white rounded-xl border border-slate-100 px-4 py-3">
                    <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[13px] font-semibold text-slate-600">
                            أنهيت {overallProgress.completed} من {overallProgress.total} مهام هذا الأسبوع
                        </span>
                ...
                <div className="mt-4 bg-gradient-to-l from-violet-50 to-purple-50 rounded-xl border border-violet-100 p-3.5 animate-fade-in">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <p className="text-[13px] font-bold text-violet-800">مراجعات اليوم</p>
                                <p className="text-[11px] text-violet-500">
                                    {srsStats.due_today > 0 && <span>{srsStats.due_today} مراجعة مستحقة</span>}
                                    {srsStats.overdue > 0 && <span className="text-rose font-semibold"> · {srsStats.overdue} متأخرة</span>}
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center gap-1 text-[11px] text-violet-500">
                            ...
                            {srsStats.total_tracked} درس متتبع
                        </div>
                    </div>
                </div>
            )}
...
function AddBlockModal({ date, onClose, onCreated }) {
    ...
    useEffect(() => {
        (async () => {
            ...
            catch { toast.error('حدث خطأ في تحميل المقررات.'); }
    ...
    const handleSubmit = async () => {
        ...
        let payload;
        if (tab === 'lesson') {
            if (!selectedLesson) { toast.error('يرجى تحديد الدرس.'); return; }
            payload = { date, type: 'lesson', lesson_id: parseInt(selectedLesson), title: currLes?.title };
        } else if (tab === 'custom') {
            if (!customTitle.trim()) { toast.error('يرجى إدخال عنوان المهمة.'); return; }
            payload = { date, type: 'custom', title: customTitle.trim() };
        } else {
            if (timerDuration < 1) { toast.error('يرجى تحديد المدة.'); return; }
            payload = { date, type: 'timer', title: timerTitle.trim() || `مؤقت ${timerDuration} دقيقة`, metadata: { duration: timerDuration } };
        }
    ...
    const tabs = [
        ...
        { key: 'custom', label: 'مهمة', icon: 'edit_note' },
        { key: 'timer', label: 'مؤقت', icon: 'timer' },
    ];
            ...
            <div className="relative w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl animate-slide-up max-h-[88vh] flex flex-col overflow-hidden">
                    ...
                    {tab === 'lesson' ? (
                        <div className="space-y-3.5">
                            {coursesLoading ? (
                                ...
                                <div className="text-center py-8"><span className="material-symbols-outlined text-slate-300" style={{ fontSize: '36px' }}>school</span><p className="text-sm text-slate-400 mt-1">لا توجد مقررات</p></div>
                            ) : (
                                <>
                                    <div>
                                        <label className="block text-[11px] font-semibold text-slate-500 mb-1">المقرر</label>
                                    ...
                                    {selectedCurriculum && currCurr?.sections?.length > 0 && (
                                        <div className="animate-fade-in">
                                            <label className="block text-[11px] font-semibold text-slate-500 mb-1">الوحدة</label>
                                    ...
                                    {selectedSection && currSec?.lessons?.length > 0 && (
                                        <div className="animate-fade-in">
                                            <label className="block text-[11px] font-semibold text-slate-500 mb-1">الدرس</label>
                        ...
                        <div>
                            <label className="block text-[11px] font-semibold text-slate-500 mb-1">عنوان المهمة</label>
                            <input type="text" value={customTitle} onChange={e => setCustomTitle(e.target.value)} placeholder="مثال: مراجعة الفصل الثالث..." className="w-full px-3 py-3 rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-800 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none placeholder:text-slate-300" maxLength={255} autoFocus onKeyDown={e => { if (e.key === 'Enter') handleSubmit(); }} />
                        </div>
                    ) : (
                        <div className="space-y-4">
                            <div>
                                <label className="block text-[11px] font-semibold text-slate-500 mb-2">مدة المؤقت</label>
                                ...
                                <div className="flex items-center gap-2 mt-3">
                                    <label className="text-[11px] text-slate-500 font-semibold flex-shrink-0">مدة محددة:</label>
                            ...
                            <div>
                                <label className="block text-[11px] font-semibold text-slate-500 mb-1">عنوان (اختياري)</label>
                                <input type="text" value={timerTitle} onChange={e => setTimerTitle(e.target.value)} placeholder="مذاكرة تركيز..." className="w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-800 focus:ring-2 focus:ring-amber/20 focus:border-amber outline-none placeholder:text-slate-300" maxLength={255} />
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50/50">
                    <button onClick={handleSubmit} disabled={submitting || !canSubmit} className={`w-full py-3 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${submitting || !canSubmit ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-primary text-white shadow-lg shadow-primary/20 hover:bg-primary-dark active:scale-[0.98]'}`}>
                        {submitting ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />جارٍ الإضافة...</> : <><span className="material-symbols-outlined" style={{ fontSize: '18px' }}>add_task</span>إضافة</>}
                    </button>
                </div>
            </div>
        </div>
...
function AIGenerateModal({ onClose, onDone }) {
    ...
    const handleGenerate = async () => {
        if (!curriculum) { toast.error('يرجى تحديد المقرر.'); return; }
        if (numDays < 1 || numDays > 30) { toast.error('عدد الأيام بين 1 و 30.'); return; }
        ...
        try {
            ...
            toast.error(err.response?.data?.message || 'حدث خطأ في إنشاء الخطة.');
        } finally { setSubmitting(false); }
    };
            ...
            <div className="relative w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl animate-slide-up max-h-[88vh] flex flex-col overflow-hidden">
                ...
                <div className="p-4 overflow-y-auto flex-1 space-y-4">
                    <p className="text-[13px] text-slate-500 leading-relaxed bg-purple-50 rounded-xl p-3 border border-purple-100">
                        <span className="font-semibold text-purple-700">كيف يعمل؟</span> — يقوم الذكاء الاصطناعي بتحليل دروس المقرر وتوزيعها بشكل متوازن على الأيام المحددة.
                    </p>
                    ...
                    {/* Num days slider */}
                    <div>
                        <label className="block text-[11px] font-semibold text-slate-500 mb-1">عدد الأيام: <span className="text-primary font-bold">{numDays}</span></label>
                        ...
                        <div className="flex justify-between text-[10px] text-slate-400 mt-0.5"><span>1 يوم</span><span>14 يوم</span></div>
                    </div>

                    {/* Start date */}
                    <div>
                        <label className="block text-[11px] font-semibold text-slate-500 mb-1">تاريخ البداية</label>
                    ...
                    <div>
                        <label className="block text-[11px] font-semibold text-slate-500 mb-2">وقت المذاكرة اليومي (اختياري)</label>
                        <div className="grid grid-cols-4 gap-2">
                            {[0, 30, 60, 90].map(m => (
                    ...
                    {curriculum && totalLessons > 0 && (
                        <div className="bg-gradient-to-l from-purple-50 to-indigo-50 rounded-xl p-3 border border-purple-100 animate-scale-in">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-[12px] font-bold text-purple-700">معاينة الخطة</span>
                            </div>
                            <p className="text-[12px] text-slate-600 leading-relaxed">
                                سيتم توزيع <strong className="text-purple-700">{totalLessons} درس</strong> على <strong className="text-purple-700">{numDays} أيام</strong>
                                {' '}(≈{perDay} {perDay > 1 ? 'دروس' : 'درس'} يومياً)
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50/50">
                    <button onClick={handleGenerate} disabled={submitting || !curriculum} className={`w-full py-3 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${submitting || !curriculum ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-gradient-to-l from-purple-600 to-indigo-600 text-white shadow-lg shadow-purple-600/20 hover:shadow-xl active:scale-[0.98]'}`}>
                        {submitting ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />جارٍ إنشاء الخطة...</> : <><span className="material-symbols-outlined" style={{ fontSize: '18px', fontVariationSettings: "'FILL' 1" }}>auto_awesome</span>إنشاء خطة ذكية</>}
                    </button>
                </div>
            </div>
        </div>
            ...
            <div className="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl animate-scale-in overflow-hidden mx-4">
                {submitted ? (
                    ...
                    <div className="p-8 text-center">
                        ...
                        <h3 className="text-lg font-bold text-slate-800 mb-2">تم تسجيل التقييم!</h3>
                        <p className="text-sm text-slate-500 mb-3">
                            المراجعة القادمة بعد <strong className="text-violet-600">{submitted.interval} {submitted.interval === 1 ? 'يوم' : 'أيام'}</strong>
                        </p>
                        <div className="flex items-center justify-center gap-4 text-[11px] text-slate-400">
                            <span>التكرار #{submitted.repetition_count}</span>
                            <span>·</span>
                            <span>معامل السهولة: {submitted.ease_factor}</span>
                        </div>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="p-5 pb-2 text-center">
                            ...
                            <h3 className="text-lg font-bold text-slate-800 mb-1">كيف كانت صعوبة الدرس؟</h3>
